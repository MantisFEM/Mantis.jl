...
MP1 = Mantis.FiniteElementSpaces.GTBSplineSpace((B1, B2), [1, -1])

# third B-spline patch
deg1 = 4;
B3 = Mantis.FiniteElementSpaces.BSplineSpace(patch1, deg1, [-1, deg1-1, -1])
# second B-spline patch
deg2 = 3;
B4 = Mantis.FiniteElementSpaces.BSplineSpace(patch2, deg2, [-1, min(deg2-1,1),  deg2-1, -1])
# First multi-patch object
MP2 = Mantis.FiniteElementSpaces.GTBSplineSpace((B3, B4), [1, -1])

# tensor-product B-spline patch
TP = Mantis.FiniteElementSpaces.TensorProductSpace((MP1, MP2), Dict())
# evaluation points
