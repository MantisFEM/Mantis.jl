...
x1_c = 3.0
x2_c = 4.0
w0 = Forms.FormField(W0)
w0.coefficients .= w0_c
w1 = Forms.FormField(W1)
w1.coefficients .= w1_c
w2 = Forms.FormField(W2)
w2.coefficients .= w2_c
x0 = Forms.FormField(X0)
x0.coefficients .= x0_c
x1 = Forms.FormField(X1)
x1.coefficients .= x1_c
x2 = Forms.FormField(X2)
...
@testset "UnaryFormTransformation" begin
    ...
    @test isapprox(Forms.evaluate(w0 * c, 1, xi)[1][1], c * w0_val)
    @test isapprox(Forms.evaluate(w1 * c, 1, xi)[1][1], c * w1_val[1])
    @test isapprox(Forms.evaluate(w1 * c, 1, xi)[1][2], c * w1_val[2])
    @test isapprox(Forms.evaluate(w2 * c, 1, xi)[1][1], c * w2_val)
    @test isapprox(Forms.evaluate(x0 * c, 1, xi)[1][1], c * x0_val)
    @test isapprox(Forms.evaluate(x1 * c, 1, xi)[1][1], c * x1_val[1])
    @test isapprox(Forms.evaluate(x1 * c, 1, xi)[1][2], c * x1_val[2])
    @test isapprox(Forms.evaluate(x2 * c, 1, xi)[1][1], c * x2_val)
    # Additive inverse
