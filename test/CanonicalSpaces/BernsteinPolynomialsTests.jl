...
for p in degrees_to_test
    ...
    b = Mantis.CanonicalSpaces.Bernstein(p)
    # ... evaluated (values, 1st and 2nd derivatives) at quadrature nodes
    b_eval = Mantis.CanonicalSpaces.evaluate(b, x, 2)
    ...
    if p > 0
        ...
        coeff_b = Mantis.CanonicalSpaces.extract_monomial_to_bernstein(b) * coeff_m
