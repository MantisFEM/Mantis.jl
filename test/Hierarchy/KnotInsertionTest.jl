...
for p in degrees_to_test
    ...
    coeffs = (rand(Mantis.FiniteElementSpaces.get_dim(coarse_bspline)) .* 2 .- 1 ) .* coeff_factor
    ...
    for nsubdivision in subdivisions_to_test
        ...
        for fine_el in 1:size(fine_bspline.knot_vector.patch_1d)
            ...
            coarse_spline_eval = Mantis.FiniteElementSpaces.evaluate(coarse_bspline, coarse_el, coarse_x, 0, coeffs)
