...
@testset "Iteration" verbose = true begin
    ...
    a1 = AnotherObject(2)
    scaling0 = Scaling(p0, a1, rel)
    scaling1 = Scaling(a1, p2, rel)
    warn_msg = Regex(join([
        "The sets provided in scalings do not form an `NTuple`. ",
        "Type-stability will be affected. ",
        "Got type Tuple\\{.*NewObject, .*AnotherObject, .*NewObject\\}"
    ]))

    @test_logs (:warn, warn_msg) Hierarchy(active, scaling0, scaling1)
end
