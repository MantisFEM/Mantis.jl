...
function Mantis.Plot.plot_solution(
    ...
    for element_id in 1:num_elements
        ...
        x = reshape(
            position_coordinates[:, 1], (plot_points_per_element, plot_points_per_element)
        )
        y = reshape(
            position_coordinates[:, 2], (plot_points_per_element, plot_points_per_element)
        )

        function_values = Forms.evaluate(field, element_id, xi)[1]
        z = reshape(function_values[1], (plot_points_per_element, plot_points_per_element))
        surface!(
            ax,
            x,
            y,
...
function _plot_topology!(
    ...
    for edge_id in 1:size(topology, 2)
        ...
        for patch_id in patch_ids
            ...
            edge_midpoint = Mantis.Plot._pad_point(
                TPoint(curved_midpoint_coordinates[2, :]...)
            )
