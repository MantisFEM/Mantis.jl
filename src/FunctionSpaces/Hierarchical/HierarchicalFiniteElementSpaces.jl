...
- `active_elements::Hierarchical.ActiveInfo`: Information about the active elements at each
    level. See [`Hierarchical.ActiveInfo`](@ref).
- `active_basis::Hierarchical.ActiveInfo`: Information about the active basis at each level.
    See [`Hierarchical.ActiveInfo`](@ref).
- `nested_domains::Hierarchical.ActiveInfo`: Information about the nested domains at each
    level. This is the usual definition of Ωₗ in the literature. See
    [`Hierarchical.ActiveInfo`](@ref).
...
struct HierarchicalFiniteElementSpace{
    ...
    active_elements::Hierarchical.ActiveInfo
    active_basis::Hierarchical.ActiveInfo
    nested_domains::Hierarchical.ActiveInfo
    ...
    function HierarchicalFiniteElementSpace(
        ...
        elseif Hierarchical.get_num_levels(nested_domains) != num_levels
            msg1 = "Number of nested domains should be the same as the number of levels. "
            msg2 = "Got $(num_levels) and $(Hierarchical.get_num_levels(nested_domains))."
            throw(ArgumentError(msg1 * msg2))
        end
...
"""
	HierarchicalFiniteElementSpace(
		spaces::Vector{S},
		two_scale_operators::Vector{T},
		domains::Hierarchical.ActiveInfo,
		num_subdivisions::NTuple{manifold_dim, Int},
		truncated::Bool=true,
...
function HierarchicalFiniteElementSpace(
    ...
    domains = Hierarchical.ActiveInfo(domains_per_level)
...
- `domains::Hierarchical.ActiveInfo`: Nested domains where the support of active basis is
    determined.

# Returns
- `active_elements::Hierarchical.ActiveInfo`: Active elements at each level.
- `active_basis::Hierarchical.ActiveInfo`: Active basis at each level.
- `nested_domains::Hierarchical.ActiveInfo`: Information about the nested domains at each
    level. This is the usual definition of Ωₗ in the literature. See
    [`Hierarchical.ActiveInfo`](@ref).
"""
function get_active_objects_and_nested_domains(
    ...
    num_levels = Hierarchical.get_num_levels(domains)
    ...
    # children as refined.
    if !simplified
        new_domains = [Hierarchical.get_level_ids(domains, level) for level in 1:num_levels]
        ...
        domains = Hierarchical.ActiveInfo(new_domains)
    end

    for level in 1:(num_levels - 1)
        next_level_domain = Hierarchical.get_level_set(domains, level + 1)
    ...
    active_elements = Hierarchical.ActiveInfo(active_elements_per_level)
    active_basis = Hierarchical.ActiveInfo(active_basis_per_level)
    nested_domains = Hierarchical.ActiveInfo(nested_domains_per_level)
...
- `two_scale_operators::Vector{AbstractTwoScaleOperator}`: two scale operators relating the
    finite element spaces at each level.
- `active_elements::Hierarchical.ActiveInfo`: active elements on each level.
- `active_basis::Hierarchical.ActiveInfo`: active basis on each level.
...
function get_multilevel_extraction(
    ...
    # First, we determine which elements contain basis functions from multiple levels.
    ...
    if num_multilevel_elements == 0
        return SparseArrays.spzeros(Int, Hierarchical.get_num_objects(active_elements)),
    ...
    for (level, element_level_id) in multilevel_keys
        ...
        basis_hier_ids = map(
            basis_level_id ->
                Hierarchical.convert_to_hier_id(active_basis, level, basis_level_id),
        ...
        multilevel_element_indices[ml_id_count] = Hierarchical.convert_to_hier_id(
    ...
    multilevel_elements = SparseArrays.sparsevec(
        ...
        Hierarchical.get_num_objects(active_elements),
    )
...
function get_multilevel_information(
    ...
    L = Hierarchical.get_num_levels(active_elements)
    ...
    # for a given level.
    for level in 1:(L - 1)
        level_active_elements = Hierarchical.get_level_set(active_elements, level)
        level_active_basis = Hierarchical.get_level_ids(active_basis, level)
        level_space = spaces[level]
        for basis in level_active_basis
            support = get_support(level_space, basis)
            for element in support
                if element in level_active_elements
                    continue
                end

                active_children = get_element_active_children(
                    active_elements, level, element, two_scale_operators
                )
...
function get_active_basis_matrix(space, element_level_id, level, active_basis)
    ...
    active_local_ids = findall(
        basis_id -> basis_id in Hierarchical.get_level_ids(active_basis, level),
        full_level_indices,
    )
...
function get_refinement_data(
    ...
    for (basis_level, basis_level_id) in multilevel_information[(element_level, element_id)]
        ...
        multilevel_basis_hier_ids[ml_basis_count] = Hierarchical.convert_to_hier_id(
...
function get_multilevel_basis_evaluation(
    ...
    if truncated
        ...
        active_indices = findall(
            basis_id -> basis_id in Hierarchical.get_level_set(active_basis, level),
            full_level_indices,
        )
        if !isempty(active_indices)
            current_subdiv_matrix = current_subdiv_matrix
            current_subdiv_matrix[active_indices, :] .= 0.0
        end
    end

    local_subdiv_matrix = copy(current_subdiv_matrix)
    current_child_element = current_parent_element
    for level in (element_level - 1):-1:(basis_level + 1)
        current_parent_element = get_element_parent(
            two_scale_operators[level - 1], current_child_element
        )
        current_subdiv_matrix = view_scaling_matrix(
            two_scale_operators[level - 1], current_parent_element, current_child_element
        )
        if truncated
            full_level_indices = get_basis_indices(fe_spaces[level], current_child_element)
            ...
            if !isempty(active_indices)
                current_subdiv_matrix = copy(current_subdiv_matrix)
                current_subdiv_matrix[active_indices, :] .= 0.0
            end
        end
...
function compute_dof_partition(spaces, active_basis, L)
    ...
    for patch in eachindex(dof_partition)
        ...
        for level in 1:L
            level_active_basis = Hierarchical.get_level_set(active_basis, level)
            for partition in eachindex(dof_partition[patch])
                ...
                dof_ids = map(
                    og_id -> Hierarchical.convert_to_hier_id(active_basis, level, og_id),
                    active_level_dofs,
                )
                if level == 1
...
"""
	refine_space(space::HierarchicalFiniteElementSpace, domains::Hierarchical.ActiveInfo)
...
- `domains::Hierarchical.ActiveInfo`: Information about the nested domains at each level. This
	is the usual definition of Ωₗ in the literature. See [`Hierarchical.ActiveInfo`](@ref).

# Returns
- `refine_space::HierarchicalFiniteElementSpace`: A refined hierarchical space.
"""
function refine_space(space::HierarchicalFiniteElementSpace, domains::Hierarchical.ActiveInfo)
    L = Hierarchical.get_num_levels(domains)
...
function refine_space(
    ...
    if !isempty(marked_elements_per_level[L])
        ...
        push!(spaces, new_space)
        push!(two_scale_operators, new_ts)
        Hierarchical.add_level!(domains)
    end
...
# Arguments
- `domains::Hierarchical.ActiveInfo`: The hierarchical domains to be refined.
...
# Returns
- `domains::Hierarchical.ActiveInfo`: The refined `domains`.
"""
...
function refine_domains!(domains, two_scale_operators, marked_elements, level)
    ...
    Hierarchical.update!(domains, level, marked_elements, refined_elements)
...
function get_num_levels(space::HierarchicalFiniteElementSpace)
    return Hierarchical.get_num_levels(get_active_elements(space))
end

function get_num_elements(space::HierarchicalFiniteElementSpace)
    return Hierarchical.get_num_objects(get_active_elements(space))
end

function get_num_basis(space::HierarchicalFiniteElementSpace)
    return Hierarchical.get_num_objects(get_active_basis(space))
end
...
function get_element_level(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.get_level(get_active_elements(space), hier_id)
end

function get_basis_level(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.get_level(get_active_basis(space), hier_id)
end

function get_element_level_id(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.get_level_ids(get_active_elements(space), hier_id)
end

function get_basis_level_id(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.get_level_ids(get_active_basis(space), hier_id)
end
...
function get_level_element_ids(space::HierarchicalFiniteElementSpace, level::Int)
    return Hierarchical.get_level_ids(get_active_elements(space), level)
end

function get_level_basis_ids(space::HierarchicalFiniteElementSpace, level::Int)
    return Hierarchical.get_level_ids(get_active_basis(space), level)
end

function get_level_domain(space::HierarchicalFiniteElementSpace, level::Int)
    return Hierarchical.get_level_ids(get_nested_domains(space), level)
end
...
function get_element_active_children(
    ...
    while !all_active_check
        ...
        for level_id in current_level_ids
            ...
            children_check =
                children .∈ [Hierarchical.get_level_set(active_elements, current_level + 1)]
...
function convert_to_element_hier_id(
    ...
    return Hierarchical.convert_to_hier_id(get_active_elements(space), level, level_id)
end

function convert_to_element_level_id(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.convert_to_level_id(get_active_elements(space), hier_id)
end

function convert_to_element_level_and_level_id(
    space::HierarchicalFiniteElementSpace, hier_id::Int
)
    return Hierarchical.convert_to_level_and_level_id(get_active_elements(space), hier_id)
end

function convert_to_basis_hier_id(
    space::HierarchicalFiniteElementSpace, level::Int, level_id::Int
)
    return Hierarchical.convert_to_hier_id(get_active_basis(space), level, level_id)
end

function convert_to_basis_level_id(space::HierarchicalFiniteElementSpace, hier_id::Int)
    return Hierarchical.convert_to_level_id(get_active_basis(space), hier_id)
end

function convert_to_basis_level_and_level_id(
    space::HierarchicalFiniteElementSpace, hier_id::Int
)
    return Hierarchical.convert_to_level_and_level_id(get_active_basis(space), hier_id)
end
...
function _compute_thb_parametric_geometry_coeffs(
    ...
    for level in 1:num_levels
        ...
        level_active_basis = Hierarchical.get_level_set(get_active_basis(space), level)
