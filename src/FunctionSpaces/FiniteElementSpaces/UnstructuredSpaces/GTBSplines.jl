...
struct GTBSplineSpace{num_patches, T, G, GP, TE, TI, TJ} <:
    ...
    function GTBSplineSpace(
        ...
        extraction_op, gtb_spline_to_b_spline_map = extract_and_relate_gtbspline_to_bspline(
            patch_spaces, regularity
        )
...
function get_support(space::GTBSplineSpace, basis_id::Int)
    ...
    function _get_bspline_support(basis_id::Int)
        ...
        return get_global_element_id.(
            Ref(space),
            Ref(patch_id),
            get_support(get_patch_spaces(space)[patch_id], basis_id),
        )
    end
...
function get_basis_integrals(
    ...
    patch_id, local_element_id = get_patch_and_local_element_id(space, element_id)
    local_basis_integrals = get_basis_integrals(
