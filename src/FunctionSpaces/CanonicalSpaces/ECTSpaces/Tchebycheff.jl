...
struct Tchebycheff <: AbstractECTSpaces
    ...
    function Tchebycheff(
        ...
        if size(roots, 1) != length(root_mult)
            throw(
                ArgumentError(
                    "The number of roots must match the number of multiplicities."
                ),
            )
        end
        ...
        if p+1 != m
            if p+1 > m
                ...
                throw(
                    ArgumentError(
                        "The local dimension, $(p+1), must be greater than or equal to the number of roots (counting multiplicities), $(m).",
                    ),
                )
            end
        end
        ...
        for ru in eachrow(roots_unq)
            root_mult_unq[count + 1] = sum(
                root_mult[findall(row -> all(row .== ru), eachrow(roots))]
            )
            count += 1
        end
        ...
        for i in eachindex(ect_space.root_mult)
            ...
            elseif ect_space.root_type[i] == 3
                ...
                for _ in 1:r
                    Ezi = copy(Eci)
                    Eci .=
                        vcat(zero_v, Eci[1:(end - 1)]) .+ ect_space.roots[i, 1] * Eci .-
                        ect_space.roots[i, 2] * Esi
                    Esi .=
                        vcat(zero_v, Esi[1:(end - 1)]) .+ ect_space.roots[i, 1] * Esi .+
                        ect_space.roots[i, 2] * Ezi
                end
                Ei = vcat(Eci, Esi)
            end
    ...
    for i in eachindex(root_mult)
        ...
        elseif r_ind == 1
            ...
            for k in 1:p
                ...
                M0i[1:(ki + 1), k + 1] .=
                    vcat(0.0, M0i[1:ki, k]) + roots[i, 1] * M0i[1:(ki + 1), k]
        ...
        elseif r_ind == 2
            ...
            for k in 1:p
                ki = min(k, n4) - 1
                M0ci[1:(ki + 1), k + 1] .=
                    vcat(0.0, M0ci[1:ki, k]) - roots[i, 2] * M0si[1:(ki + 1), k]
                M0si[1:(ki + 1), k + 1] .=
                    vcat(0.0, M0si[1:ki, k]) + roots[i, 2] * M0ci[1:(ki + 1), k]
        ...
        elseif r_ind == 3
            ...
            for k in 1:p
                ki = min(k, n4) - 1
                M0ci[1:(ki + 1), k + 1] .=
                    vcat(0.0, M0ci[1:ki, k]) + roots[i, 1] * M0ci[1:(ki + 1), k] -
                    roots[i, 2] * M0si[1:(ki + 1), k]
                M0si[1:(ki + 1), k + 1] .=
                    vcat(0.0, M0si[1:ki, k]) +
                    roots[i, 1] * M0si[1:(ki + 1), k] +
                    roots[i, 2] * M0ci[1:(ki + 1), k]
                M1ci[:, k + 1] .=
                    vcat(0.0, M1ci[1:(end - 1), k]) + roots[i, 1] * M1ci[:, k] -
                    roots[i, 2] * M1si[:, k]
                M1si[:, k + 1] .=
                    vcat(0.0, M1si[1:(end - 1), k]) +
                    roots[i, 1] * M1si[:, k] +
                    roots[i, 2] * M1ci[:, k]
            end
            M0i = vcat(M0ci, M0si)
...
function get_derivative_space(ect_space::Tchebycheff)
    ...
    return Tchebycheff(ect_space.p-1, new_roots, ect_space.l, new_root_mult)
end
