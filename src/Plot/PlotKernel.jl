...
function _plot(geometry::G, field::Union{Nothing, F} = nothing; vtk_filename::String = "default", n_subcells::Int64 = 1, degree::Int64 = 1) where {G <: AbstractGeometry, F <: AbstractFunction}
    ...
    n_vertices = prod(geometry.n_elements) * (n_subcells^2) * ((degree+1)^2)  # there are (n_subcells+ 1)^2 cells per element, and (p+1)^2 vertices per cell
    ...
    vertices = Array{Float64, 2}(undef, 2, n_vertices)
