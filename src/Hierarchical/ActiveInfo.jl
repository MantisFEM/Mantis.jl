        ...
        if check_unique
            for l in eachindex(level_ids)
                if !allunique(level_ids[l])
                    throw(
                        ArgumentError(
                            "Level indices must be unique. Failed on level $(l).",
                        ),
                    )
                end
            end
...
Returns a modified copy of `active_info` after updating the active object identifiers.
...
function update(active_info::ActiveInfo, level::Int, remove::Vector{Int}, add::Vector{Int})
    # Deep-copy to protect the original from the mutation happening in the rest of the
    # method
    ...
    get_level_lookup(active_info)[level] =
        Dict(id => i for (i, id) in enumerate(get_level_ids(active_info, level)))
    get_level_lookup(active_info)[level+1] =
        Dict(id => i for (i, id) in enumerate(get_level_ids(active_info, level + 1)))

    # The previous changes to `level` and `level+1` require updating the cumulative count
    # for all subsequent levels
    level_cum_num_ids = get_level_cum_num_ids(active_info)
    for l = level:num_levels
...
Returns a _modified_ `active_info` after appending an empty level.
"""
...
Mutation-safe version of [`add_level!`](@ref).
"""
function add_level(active_info::ActiveInfo)
    active_info = deepcopy(active_info)
    active_info = add_level!(active_info)
