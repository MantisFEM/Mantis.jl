...
# TODO: Would be nice to have a `coarsen` method as well.
...
function refine(
    ...
    add = mapreduce(p -> collect(get_children(scaling, p)), vcat, remove)
...
function update(
    ...
    # It's okay to call mutating methods here since `update` already deep-copies
    # `active_info`
...
#=
    WARNING:
    This operation is inherently type unstable as it depends on the runtime value of `L`.
    However, it is limited to a Union of 2
    options:
    ...
    See the example below the next method.
=#
@noinline function _maybe_add_level(
    ...
    if L < get_num_levels(active_info)
        ...
        return constructor(active_info, get_scalings(hierarchy)...)
    end
end
...
struct Hierarchy{L,S} <: AbstractHierarchy{L,S}
    ...
    function Hierarchy(
        active_info::ActiveInfo,
        scalings::Vararg{AbstractScaling,LS},
    ) where {LS}
        for i = 1:(LS-1)
        ...
        if !(get_sets(scalings) isa NTuple)
            @warn LazyString(
                "The sets provided in scalings do not form an `NTuple`. ",
                "Type-stability will be affected. ",
                "Got type ",
                typeof(get_sets(scalings)),
            )
        end
