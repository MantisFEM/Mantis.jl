...
function evaluate(geometry::FEMGeometry{n,n}, element_idx::Int, ξ::Vector{Float64}) where {n}
    return evaluate(geometry, element_idx, ntuple(i -> ξ, n))
end
...
function jacobian(geometry::FEMGeometry{n,n}, element_idx::Int, ξ::Vector{Float64}) where {n}
    return jacobian(geometry, element_idx, ntuple(i -> ξ, n))
end
