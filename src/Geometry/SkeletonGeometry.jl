...
) where {patch_dim, image_dim, num_patches}
...
function get_parent_elements(
    geometry::SkeletonGeometry{patch_dim, image_dim, num_patches}, element_id::Int
) where {patch_dim, image_dim, num_patches}
...
function skeleton_element_to_parent_element_coords(
    ...
    for position_id in 1:(manifold_dim + 1)
        if position[position_id] == 0
            ...
            constituent_points[position_id] = single_coordinate_of_type(
                (position[position_id] + 1.0)/2.0, eltype(σ)
            )
        end
    end
    ...
    return Points.TensorProductPoints(constituent_points...)
end
...
function skeleton_element_to_parent_element_coords(
    ...
    if orientation == -1
        iteration_order_rotation[1], iteration_order_rotation[2] = iteration_order_rotation[2],
        iteration_order_rotation[1]
    ...
    for position_id in 1:(manifold_dim + 1)
        if position[position_id] == 0
            ...
            # Rescale to 0 or 1 instead of -1 or 1 as in position
            constituent_points[position_id] = single_coordinate_of_type(
                (position[position_id] + 1.0)/2.0, eltype(σ_τ)
            )
        end
    end
    ...
    return Points.TensorProductPoints(
        constituent_points...; iteration_order=Tuple(iteration_order)
    )
end
...
function single_coordinate_of_type(coordinate::F, ::Type{Vector{F}}) where {F <: Real}
    return [coordinate]
end
