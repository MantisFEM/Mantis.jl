...
function tensor_product_rule(
    ...
    factor_qrules = NTuple{manifold_dim, CanonicalQuadratureRule{1}}(
        quad_rule(p[k], rule_args_1d...) for k in 1:manifold_dim
    )

    return tensor_product_rule(factor_qrules)
end
...
function tensor_product_rule(
    ...
    factor_nodes = ntuple(
        dim -> Points.get_input_points(get_nodes(factor_qrules[dim]))[1], manifold_dim
    )
    nodes = Points.TensorProductPoints(factor_nodes)
    factor_weights = NTuple{manifold_dim, Vector{Float64}}(
        get_weights(factor_qrules[k]) for k in 1:manifold_dim
    )
    weights = _compute_tensor_product(factor_weights)
    ...
    if manifold_dim == 1
        rule_label = get_label(factor_qrules[1])
    else
        rule_labels = join([get_label(factor_qrules[i]) for i in 1:manifold_dim], ", ")
