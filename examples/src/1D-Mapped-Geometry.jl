...
@show Geometry.evaluate(original_geo, le, Points.PointSet(([1.0],)))[1] == lp
...
@show Geometry.hessian(original_geo, 1, Points.PointSet(([0.0],)))[1][1][1] == 0.0
@show Geometry.hessian(original_geo, 3, Points.PointSet(([1.0],)))[1][1][1] == 0.0
# We have the first ingredient needed to build a `MappedGeometry`; now we just need to
# define what our mapping $\phi$ is, using the structure `Geometry.Mapping`. This strucutre
# will contain the definition of $\phi$, $\frac{d\phi}{d\hat{\xi}}$ and, optionally,
# $\frac{d^2\phi}{d\hat{\xi}^2}$. Because we will choose a simple mapping we can provide
# both the first and second derivatives. (The argument `(1, 1)` in the function call just
# means that both our original and mapped geometries can be represented in just 1
# dimension.)
d = 3
ϕ_map(ξ̂) = ξ̂^d
dϕ_map(ξ̂) = d * ξ̂^(d - 1)
d2ϕ_map(ξ̂) = d * (d - 1) * ξ̂^(d - 2)
ϕ = Geometry.Mapping(
...
@show Geometry.jacobian(geo, le, Points.PointSet(([1.0],)))[1][1] == dϕ_map(lp) * dφᵢ
@show Geometry.hessian(geo, 1, Points.PointSet(([0.0],)))[1][1][1] == d2ϕ_map(fp) * dφᵢ^2
@show Geometry.hessian(geo, le, Points.PointSet(([1.0],)))[1][1][1] == d2ϕ_map(lp) * dφᵢ^2
