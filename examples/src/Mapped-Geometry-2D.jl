...
# (see [One-dimensional mapped geometry](@ref)):
...
Geometry.evaluate(geo, 1, p_origin) ≈ [r₀ 0.0]

# Last element at ``\xi=(1,1)`` should give ``(0, r_1)``. (The ``x``-component is
# ``r_1\cos(\pi/2)``, i.e. zero up to floating-point round-off, so we compare the whole point.)
Geometry.evaluate(geo, nr * nθ, p_corner) ≈ [0.0 r₁]
